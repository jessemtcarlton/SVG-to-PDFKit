...
var SVGtoPDF = (function() {
  ...
  function generateSVGObject(doc, svg, options) {
    ...
    function docEndGroup(group) {
      if (group !== doc._currentGroup || group.closed) {
        throw new Error("Group not matching or already closed");
      }
        ...
        return (
          (function recursive(node, id) {
            ...
            if (node.nodeType === 1) {
              ...
              for (let i = 0; i < node.childNodes.length; i++) {
                if ((temp = recursive(node.childNodes[i], id))) {
                  return temp;
                }
              }
            }
    ...
    function transformBBox(bbox, m) {
      return [
        m[0] * bbox[0] + m[2] * bbox[1] + m[4],
        m[1] * bbox[0] + m[3] * bbox[1] + m[5],
        m[0] * bbox[2] + m[2] * bbox[3] + m[4],
        m[1] * bbox[2] + m[3] * bbox[3] + m[5]
      ];
    }
    ...
    var LineSegment = function(p1x, p1y, p2x, p2y) {
      let totalLength = (this.totalLength = Math.sqrt(
    ...
    var SvgPath = function(d) {
      ...
      while ((command = parser.match(/^[astvzqmhlcASTVZQMHLC]/))) {
        ...
        values = [];
        while (
          (value = Flags[command + values.length]
            ? parser.match(/^[01]/)
            : parser.matchNumber())
        ) {
          parser.matchSeparator();
    ...
    var SvgShape = function(commands) {
      ...
      Object.defineProperty(this, "startPoint", {
        get: function() {
          ...
          return (startPoint =
            this.pathSegments.length && this.pathSegments[0].startPoint);
        }
      });
      let endPoint = null;
    ...
    var SvgElem = function(obj, inherits) {
      ...
      this.inherits =
        inherits ||
        (!this.isOuterElement ? new SvgElem(obj.parentNode, null) : null);
          ...
          switch (key) {
            ...
            case "stroke":
              result = function() {
                ...
                } // and it returns null if the syntax looks good but the element was not found
                if (
    ...
    var SvgElemLink = function(obj, inherits) {
      ...
      this.drawInDocument = function(isClip, isMask) {
        doc.save();
        if (link && this.getChildren().length) {
    ...
    var SvgElemGradient = function(obj, inherits, fallback) {
      ...
      this.getPaint = function(bBox, gOpacity, isClip, isMask) {
        ...
        if (children.length === 1) {
          ...
          return [
            uniqueColor.slice(0, 3),
            child.get("stop-opacity") * uniqueColor[3] * gOpacity
          ];
        }
        ...
        if ((matrix = validateMatrix(matrix))) {
          ...
          if (spread === "reflect" || spread === "repeat") {
            ...
            if (this.name === "linearGradient") {
              nAfter =
                Math.max(
                  (tbbox[0] - x2) * (x2 - x1) + (tbbox[1] - y2) * (y2 - y1),
              ...
              nBefore =
                Math.max(
                  (tbbox[0] - x1) * (x1 - x2) + (tbbox[1] - y1) * (y1 - y2),
              ...
              nAfter =
                Math.sqrt(
                  Math.max(
                    Math.pow(tbbox[0] - x2, 2) + Math.pow(tbbox[1] - y2, 2),
                    Math.pow(tbbox[0] - x2, 2) + Math.pow(tbbox[3] - y2, 2),
                    Math.pow(tbbox[2] - x2, 2) + Math.pow(tbbox[1] - y2, 2),
                    Math.pow(tbbox[2] - x2, 2) + Math.pow(tbbox[3] - y2, 2)
                  )
                ) / r2;
            }
    ...
    var SvgElemPath = function(obj, inherits) {
      ...
      this.pathLength =
        Math.max(0, this.getLength("pathLength", this.getViewport(), 0)) ||
        undefined;
      this.dashScale =
              ...
              case "#text":
                ...
                for (let w = 0; w < words.length; w++) {
                  ...
                  for (let j = 0; j < pos.length; j++) {
                    ...
                    if (currentElem._x[index] !== undefined) {
                      doAnchoring();
                      currentX = currentElem._x[index];
                    }
                    if (currentElem._y[index] !== undefined) {
                      doAnchoring();
                      currentY = currentElem._y[index];
                    }
                    ...
                    let position = {
                      ...
                      rotate:
                        Math.PI /
                        180 *
                        currentElem.chooseValue(
                          currentElem._rot[index],
                          currentElem._defRot
                        ),
        ...
        function textOnPath(currentElem) {
          ...
          if (pathElem) {
            let pathObject = pathElem.shape
              ...
              pathLength = pathElem.chooseValue(
                pathElem.pathLength,
                pathComputedLength
              ),
              pathLengthScale = pathComputedLength / pathLength,
              textOffset =
                currentElem.getLength("startOffset", pathLength, 0) *
                pathLengthScale;
